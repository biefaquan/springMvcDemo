...
import com.bfq.mvc.util.Play;
import com.bfq.mvc.vo.HandlerModel;
/**
 * 自定义的SpringMVC的前端控制器
 * @author bfq
 *
 */
public class DispatcherSertvlet extends HttpServlet {
	//存储类名
			...
			try {
				...
				if(clazz.isAnnotationPresent(Controller.class)){
					...
					if(!"".equals(value.trim())){
						...
						//此处做简单处理，假设只实现一个接口
						for(Class c : inters){
							//举例 modifyService->new ModifyServiceImpl（） ,实现填写impl类
	...
	private void doAutoWired(){
		...
		for(Map.Entry<String, Object> entry : instanceMapping.entrySet()){
			...
			for (Field field : fields) {
				...
				Autowired autwired = field.getAnnotation(Autowired.class);
				if("".equals(autwired.value())){
				 ...
				 for (int i = 0; i < annotations.length; i++) {
					//获取每个参数上面的所有注解
					 ...
					 for (Annotation ans : anns) {
						//找到被@RequestParam注解的参数，并取value值
			...
			if(entry.getKey().equals(requestUri)){
				...
				for(Map.Entry<String, Integer> param : paramIndexMap.entrySet()){
					...
					if(key.equals(HttpServletRequest.class.getName())){
						...
						//如果用户传了参数，譬如 name = "wolf"，做一下参数类型转换，将用户传来的值转换为方法中参数的类型
	 ...
	 */
	private Object convert(String paramter, Class<?> targetType){
		if(targetType == String.class){
			return paramter;
		}else if(targetType == Integer.class || targetType == int.class){
			return Integer.valueOf(paramter);
		}else if(targetType == Long.class || targetType == long.class){
			return Long.valueOf(paramter);
		}else if(targetType == Boolean.class || targetType == boolean.class){
			if (paramter.toLowerCase().equals("true") || paramter.equals("1")) {  
                return true;  
            } else if (paramter.toLowerCase().equals("false") || paramter.equals("0")) {  
                return false;  
            }  
